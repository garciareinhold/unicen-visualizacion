...
console.log(document.getElementById('p1').value);
  ...
  $('.images').click("bind", function(event) {
    ...
    if (canPickThatImg(value, imgSource)) {
      (value == "red")
        ? player1.image = imgSource
        : player2.image = imgSource;
  ...
  function canPickThatImg(value, src) {
    if ((value == "red" && player2.image != src) || (value == "green" && player1.image != src))
      return true;
    else
      return false;
    }
  ...
  function winner() {
    if (fourBelow()) {
      winPlay = "Four Vertically";
      return true;
    } else if (fourOnSides()) {
      winPlay = "Four Horizontally";
      return true;
